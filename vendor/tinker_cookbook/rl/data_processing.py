...
def trajectory_to_data(traj: Trajectory, traj_advantage: float | list[float]) -> list[tinker.Datum]:
    """
    ...
    # Support per-token advantages: if traj_advantage is a list, pop tokens from it
    step_level = isinstance(traj_advantage, list)
    token_offset = 0  # tracks position in the per-token advantage list
    ...
    for transition in traj.transitions:
        ...
        n_action_tokens = len(ac_with_logprobs.tokens)
        if step_level:
            # Per-token advantages from the list
            tok_advs = traj_advantage[token_offset:token_offset + n_action_tokens]
            # Pad if needed (shouldn't happen but be safe)
            while len(tok_advs) < n_action_tokens:
                tok_advs.append(tok_advs[-1] if tok_advs else 0.0)
            SequenceAccumulator.advantages.extend([0] * delta_ob_len + tok_advs)
            token_offset += n_action_tokens
        else:
            SequenceAccumulator.advantages.extend(
                [0] * delta_ob_len + [traj_advantage] * n_action_tokens
            )
        SequenceAccumulator.mask.extend([0.0] * delta_ob_len + [1.0] * n_action_tokens)
...
def assemble_training_data(
    ...
    advantages_P: List[torch.Tensor] | List,
) -> tuple[List[tinker.Datum], List[dict[str, int]]]:
    """Convert trajectories to training data format.

    advantages_P can be:
    - List[torch.Tensor]: standard GRPO scalar advantages per trajectory
    - Mixed list of torch.Tensor | list[list[float]]: RAPO step-level advantages
    """
    ...
    for i_group, (traj_group, advantages_G) in enumerate(
        ...
        if isinstance(advantages_G, torch.Tensor):
            # Standard GRPO: scalar advantage per trajectory
        ...
        elif isinstance(advantages_G, list):
            # RAPO: per-token advantages per trajectory
            for i_traj, (traj, tok_advantages) in enumerate(
                safezip(traj_group.trajectories_G, advantages_G)
            ):
                new_data = trajectory_to_data(traj, tok_advantages)
                data_D.extend(new_data)
                metadata_D.extend([dict(group_idx=i_group, traj_idx=i_traj) for _ in new_data])
        else:
            raise TypeError(f"Unexpected advantage type: {type(advantages_G)}")
