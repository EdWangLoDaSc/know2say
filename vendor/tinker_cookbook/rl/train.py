...
class Config:
    ...
    # -------------------------------------------------------------------------
    # RAPO (Reachability-Aware Policy Optimization) configuration
    # -------------------------------------------------------------------------
    # Set to enable step-level credit assignment via reachability probes.
    rapo_config: RAPOConfig | None = None
...
async def prepare_minibatch(
    ...
    with timed("assemble_training_data", metrics):
        if rapo_config is not None and sampling_client is not None and grade_fn is not None and parse_fn is not None:
            # RAPO: step-level credit assignment via reachability probes
            from tinker_cookbook.rl.rapo import (
                collect_probe_phis,
                compute_rapo_advantages,
                get_alpha,
                submit_group_probes,
            )

            alpha = get_alpha(i_batch, num_batches, rapo_config)
            metrics["rapo/alpha"] = alpha

            # Submit all probes in parallel
            all_probe_requests = []
            group_grade_fns = []
            for traj_group, env_builder in zip(trajectory_groups_P, env_group_builders_P):
                rewards = traj_group.get_total_rewards()
                all_same = all(r == rewards[0] for r in rewards)
                # Get grade function from env builder if available
                gfn = getattr(env_builder, 'get_grade_fn', lambda: grade_fn)()
                group_grade_fns.append(gfn)
                # Probe mixed groups and all-incorrect groups
                if (not all_same or all(r == 0 for r in rewards)) and gfn is not None:
                    probes = submit_group_probes(
                        sampling_client, traj_group, rapo_config, max_tokens, stop,
                    )
                    all_probe_requests.append(probes)
                else:
                    all_probe_requests.append([])

            # Collect probe results
            probe_results_P: list[list[tuple[int, float]] | None] = []
            n_probed = 0
            for probes, gfn in zip(all_probe_requests, group_grade_fns):
                if probes and gfn is not None:
                    n_probed += 1
                    probe_results_P.append(collect_probe_phis(probes, gfn, parse_fn))
                else:
                    probe_results_P.append(None)
            metrics["rapo/n_probed"] = n_probed

            advantages_P = compute_rapo_advantages(
                trajectory_groups_P, probe_results_P, rapo_config, alpha,
            )
        else:
...
async def do_train_step_and_get_sampling_client(
    ...
    sampling_client: tinker.SamplingClient | None = None,
    num_batches: int = 1,
    rapo_fns: dict | None = None,
) -> tuple[tinker.SamplingClient, dict[str, Any]]:
    update_scope_context({"step": i_batch})

    metrics = {}
    # Build RAPO kwargs if configured
    rapo_kwargs: dict[str, Any] = {}
    if cfg.rapo_config is not None and sampling_client is not None and rapo_fns is not None:
        rapo_kwargs = {
            "rapo_config": cfg.rapo_config,
            "sampling_client": sampling_client,
            "i_batch": i_batch,
            "num_batches": num_batches,
            "grade_fn": rapo_fns.get("grade_fn"),
            "parse_fn": rapo_fns.get("parse_fn"),
            "stop": rapo_fns.get("stop"),
            "max_tokens": cfg.max_tokens,
        }
...
async def do_sync_training(
    ...
    # RAPO setup: grade_fn and parse_fn are passed via rapo_fns dict if configured
    rapo_fns: dict | None = None
    if cfg.rapo_config is not None:
        from tinker_cookbook import renderers as _renderers
        from tinker_cookbook import model_info as _model_info
        renderer_name = cfg.renderer_name or _model_info.get_recommended_renderer_name(cfg.model_name)
        renderer = _renderers.get_renderer(renderer_name, tokenizer=tokenizer)
        stop = renderer.get_stop_sequences()

        def _parse_fn(tokens: list[int]) -> str:
            msg, _ = renderer.parse_response(tokens)
            return _renderers.get_text_content(msg)

        rapo_fns = {
            "parse_fn": _parse_fn,
            "grade_fn": None,  # Will be set by env-specific code
            "stop": stop,
        }
        logger.info(f"RAPO enabled: alpha_max={cfg.rapo_config.alpha_max}, "
                     f"n_probes={cfg.rapo_config.n_probes}")
